...
class _PerfilPageState extends State<PerfilPage> with TickerProviderStateMixin {
  ...
  void _cargarFormulario(MiPerfilPaciente perfil) {
    _nombresController.text = perfil.nombres;
    _apellidosController.text = perfil.apellidos;
    _telefonoController.text = perfil.telefono ?? '';
    _contactoEmergenciaController.text = perfil.contactoEmergencia ?? '';
    _direccionController.text = perfil.direccion ?? '';
    _fechaNacimiento = perfil.fechaNacimiento;
    _fechaController.text = _formatoFecha(perfil.fechaNacimiento);
    _sexo = perfil.sexo.isEmpty ? null : perfil.sexo;
  }

  void _iniciarEdicion() {
    final perfil = _perfil;
    if (perfil == null || _guardando) return;
    _cargarFormulario(perfil);
    setState(() => _editando = true);
  }

  void _cancelarEdicion() {
    if (_perfil != null) _cargarFormulario(_perfil!);
  ...
  Future<void> _guardarCambios() async {
    ...
    final datos = MiPerfilPacienteActualizar(
    ...
    try {
      final perfilActualizado = await _pacienteService.actualizarMiPerfil(
        datos,
      );
      if (!mounted) return;
      setState(() {
      ...
      _cargarFormulario(perfilActualizado);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Tu perfil se actualizó correctamente.'),
  ...
  Widget _construirContenido() {
    ...
    if (_editando) return _buildFormularioEdicion();
  ...
  Widget _campoEdicion(
    ...
    return TextFormField(
      ...
      validator: (valor) {
        if (required && (valor == null || valor.trim().isEmpty)) {
  ...
  Future<void> _seleccionarFecha() async {
    ...
    if (fecha != null && mounted) {
      setState(() {
        ...
        _fechaController.text = _formatoFecha(fecha);
      });
    }
  }

  Widget _campoSexoEdicion() {
    return DropdownButtonFormField<String>(
      ...
      onChanged: _guardando ? null : (value) => setState(() => _sexo = value),
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                const Icon(Icons.logout, size: 20, color: Color(0xFFD32F2F)),
